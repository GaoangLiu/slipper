...
class Stock():
    ...
    def trend(self, sid: str):
        ...
        def calculate_ratio(r: float, s: float) -> str: # r calculated agains s
            x = float(r) / float(s) - 1
            return colored(str(round(x * 100, 4)), 'green') if x < 0 else colored('+' + str(round(x * 100, 4)), 'red')

        print("{:<25}: {:<15}".format("Time", date + " / " + time))
        print("{:<25}: {:<15}".format("Stock name", name))
        print("{:<25}: {}".format("Previous close price", pcp))
        print("{:<25}: {} ({}%)".format("Today open price", top, calculate_ratio(top, pcp)))
        print("{:<25}: {} ({}%) 👈 ".format("Current price", cp, calculate_ratio(cp, pcp)))
        print("{:<25}: {} ({}%)".format("Today highest price", thp, calculate_ratio(thp, pcp)))
        print("{:<25}: {} ({}%)".format("Today lowest price", tlp, calculate_ratio(tlp, pcp)))
        # print('-'*66)

        top, tlp, thp = float(top), float(tlp), float(thp)
        print("{:<25}: {:<2.3f} ".format("Suggested ASK price", round(thp * 0.1 + tlp * 0.9, 3)))
        print("{:<25}: {:<2.3f} ".format("Suggested BID price", round(tlp * 0.1 + thp * 0.9, 3) ))
        ...
        for i in range(0, 10, 2):
            print("{:>25} {:<10}".format( top5buyers[i+1], int(top5buyers[i]) // 100 ))
        
        print("Top five bids: ")
        for i in range(0, 10, 2):
            print("{:>25} {:<10}".format( top5sellers[i+1], int(top5sellers[i]) // 100 ))
